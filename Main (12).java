...
public class Main {
    ...
    static int precedence(char ch) {
        switch (ch) {
            case '|':
            case '&':
                return 1; // Logical OR and AND have lowest precedence
            case '=':
            case '<':
            case '>':
                return 2; // Relational operators have higher precedence than logical operators
            case '+':
            case '-':
                return 3;
            case '*':
    ...
    static String infixToPostfix(String expression) {
        ...
        for (int i = 0; i < expression.length(); i++) {
            ...
            System.out.printf("%-20s%-20s%-20s%n", c, stack.isEmpty() ? "# " : "#" + stack.peek(), result.toString());
        }
